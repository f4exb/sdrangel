...
void AMModSettings::applySettings(const QStringList& settingsKeys, const AMModSettings& settings)
{
    ...
    if (settingsKeys.contains("modFactor")) {
...
QString AMModSettings::getDebugString(const QStringList& settingsKeys, bool force) const
{
    ...
    if (settingsKeys.contains("modFactor") || force) {
        ostr << " m_modFactor: " << m_modFactor;
    }
