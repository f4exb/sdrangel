...
void MeshtasticModSource::initSF(unsigned int sf)
{
    m_fftLength = 1 << sf;
    m_state = ChirpChatStateIdle;
    m_quarterSamples = (m_fftLength/4)*MeshtasticModSettings::oversampling;

    float halfAngle = M_PI/MeshtasticModSettings::oversampling;
    ...
    m_phaseIncrements = new double[2*m_fftLength*MeshtasticModSettings::oversampling];
    phase = -halfAngle;

    for (unsigned int i = 0; i < m_fftLength*MeshtasticModSettings::oversampling; i++)
    {
        m_phaseIncrements[i] = phase;
        phase += (2*halfAngle) / (m_fftLength*MeshtasticModSettings::oversampling);
    }

    std::copy(
        m_phaseIncrements,
        m_phaseIncrements+m_fftLength*MeshtasticModSettings::oversampling,
        m_phaseIncrements+m_fftLength*MeshtasticModSettings::oversampling
    );
}

void MeshtasticModSource::initTest(unsigned int sf, unsigned int deBits)
{
...
void MeshtasticModSource::reset()
{
    m_chirp = 0;
    m_chirp0 = 0;
    m_sampleCounter = 0;
...
void MeshtasticModSource::pull(SampleVector::iterator begin, unsigned int nbSamples)
{
    std::for_each(
        begin,
        begin + nbSamples,
...
void MeshtasticModSource::pullOne(Sample& sample)
{
...
void MeshtasticModSource::modulateSample()
{
    if (m_state == ChirpChatStateIdle)
    {
        ...
        if (m_sampleCounter == m_quietSamples*MeshtasticModSettings::oversampling) // done with quiet time
        {
            m_chirp0 = 0;
    ...
    if (m_chirp >= (m_chirp0 + m_fftLength)*MeshtasticModSettings::oversampling) {
        m_chirp = m_chirp0*MeshtasticModSettings::oversampling;
    }
}
...
void MeshtasticModSource::applySettings(const MeshtasticModSettings& settings, bool force)
{
...
void MeshtasticModSource::applyChannelSettings(int channelSampleRate, int bandwidth, int channelFrequencyOffset, bool force)
{
    qDebug() << "MeshtasticModSource::applyChannelSettings:"
            << " channelSampleRate: " << channelSampleRate
            << " channelFrequencyOffset: " << channelFrequencyOffset
            << " bandwidth: " << bandwidth
            << " SR: " << bandwidth * MeshtasticModSettings::oversampling;
     ...
     || (bandwidth != m_bandwidth) || force)
    {
        ...
        m_interpolatorDistance = (Real) (bandwidth*MeshtasticModSettings::oversampling) / (Real) channelSampleRate;
...
void MeshtasticModSource::setSymbols(const std::vector<unsigned short>& symbols)
{
    m_symbols = symbols;
    qDebug("MeshtasticModSource::setSymbols: m_symbols: %lu", m_symbols.size());
    m_repeatCount = m_settings.m_messageRepeat;
    m_state = ChirpChatStateIdle; // first reset to idle
    reset();
    m_sampleCounter = m_quietSamples*MeshtasticModSettings::oversampling - 1; // start immediately
}
