...
VORGUI::VORGUI(NavAid *navAid, VORDemodSCGUI *gui) :
    m_navAid(navAid),
    m_gui(gui)
{
...
void VORDemodSCGUI::resizeTable()
{
...
void VORDemodSCGUI::vorData_sectionMoved(int logicalIndex, int oldVisualIndex, int newVisualIndex)
{
    (void) oldVisualIndex;
...
void VORDemodSCGUI::vorData_sectionResized(int logicalIndex, int oldSize, int newSize)
{
    (void) oldSize;
...
void VORDemodSCGUI::columnSelectMenu(QPoint pos)
{
    menu->popup(ui->vorData->horizontalHeader()->viewport()->mapToGlobal(pos));
}

// Hide/show column when menu selected
void VORDemodSCGUI::columnSelectMenuChecked(bool checked)
{
    (void) checked;
...
QAction *VORDemodSCGUI::createCheckableItem(QString &text, int idx, bool checked)
{
...
void VORDemodSCGUI::selectVOR(VORGUI *vorGUI, bool selected)
{
    if (selected)
    {
...
bool VORDemodSCGUI::handleMessage(const Message& message)
{
    ...
    else if (VORDemodSCReport::MsgReportFreqOffset::match(message))
    {
        VORDemodSCReport::MsgReportFreqOffset& report = (VORDemodSCReport::MsgReportFreqOffset&) message;
        int subChannelId = report.getSubChannelId();
...
qint64 VORDemodSCGUI::fileAgeInDays(QString filename)
{
    QFile file(filename);
    if (file.exists())
    {
...
bool VORDemodSCGUI::confirmDownload(QString filename)
{
...
QString VORDemodSCGUI::getDataDir()
{
...
QString VORDemodSCGUI::getOpenAIPVORDBFilename(int i)
{
...
QString VORDemodSCGUI::getOpenAIPVORDBURL(int i)
{
    if (countryCodes[i] != nullptr)
...
QString VORDemodSCGUI::getVORDBFilename()
{
    return getDataDir() + "/vorDatabase.csv";
}

void VORDemodSCGUI::updateDownloadProgress(qint64 bytesRead, qint64 totalBytes)
{
    m_progressDialog->setMaximum(totalBytes);
    m_progressDialog->setValue(bytesRead);
}

void VORDemodSCGUI::downloadFinished(const QString& filename, bool success)
{
    if (success)
    {
        ...
        {
            qDebug() << "VORDemodSCGUI::downloadFinished: Unexpected filename: " << filename;
            m_progressDialog->close();
            m_progressDialog = nullptr;
        }
    }
    ...
    {
        qDebug() << "VORDemodSCGUI::downloadFinished: Failed: " << filename;
        m_progressDialog->close();
        m_progressDialog = nullptr;
...
void VORDemodSCGUI::on_getOurAirportsVORDB_clicked(bool checked)
{
    (void) checked;
...
void VORDemodSCGUI::on_getOpenAIPVORDB_clicked(bool checked)
{
    (void) checked;
...
void VORDemodSCGUI::readNavAids()
{
...
void VORDemodSCGUI::on_magDecAdjust_clicked(bool checked)
{
    ...
    m_vors(nullptr)
{
    ...
    connect(&m_dlm, &HttpDownloadManager::downloadComplete, this, &VORDemodSCGUI::downloadFinished);
