...
#ifndef PLUGINS_CHANNELRX_DEMODMESHTASTIC_MESHTASTICDEMODSETTINGS_H_
#define PLUGINS_CHANNELRX_DEMODMESHTASTIC_MESHTASTICDEMODSETTINGS_H_
...
struct MeshtasticDemodSettings
{
    enum CodingScheme
    {
    ...
    QString m_meshtasticKeySpecList; //!< Optional per-channel Meshtastic decode key list
    bool m_meshtasticAutoSampleRate; //!< Auto-tune source sample rate/decimation for selected Meshtastic profile
    QString m_meshtasticRegionCode;  //!< UI-selected Meshtastic region code (US, EU_868, ...)
    QString m_meshtasticPresetName;  //!< UI-selected Meshtastic preset (LONG_FAST, ...)
    int m_meshtasticChannelIndex;    //!< UI-selected Meshtastic channel index (zero-based)
    uint32_t m_rgbColor;
    QString m_title;
    int m_streamIndex;
    ...
    void applySettings(const QStringList& settingsKeys, const MeshtasticDemodSettings& settings);
    QString getDebugString(const QStringList& settingsKeys, bool force=false) const;
};



#endif /* PLUGINS_CHANNELRX_DEMODMESHTASTIC_MESHTASTICDEMODSETTINGS_H_ */
