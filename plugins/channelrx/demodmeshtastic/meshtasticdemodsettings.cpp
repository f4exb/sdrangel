...
const int MeshtasticDemodSettings::bandwidths[] = {
    325,    // 384k / 1024
    488,    // 500k / 1024
    750,    // 384k / 512
    1500,   // 384k / 256
...
const int MeshtasticDemodSettings::nbBandwidths = 3*8 + 4;
...
void MeshtasticDemodSettings::resetToDefaults()
{
    m_bandwidthIndex = 5;
    m_spreadFactor = 7;
    m_deBits = 0;
    ...
    m_udpPort = 9999;
    m_meshtasticKeySpecList.clear();
    m_meshtasticAutoSampleRate = true;
    m_meshtasticRegionCode = "US";
    m_meshtasticPresetName = "LONG_FAST";
    m_meshtasticChannelIndex = 0;
    m_rgbColor = QColor(255, 0, 255).rgb();
    m_title = "Meshtastic Demodulator";
    m_streamIndex = 0;
...
QByteArray MeshtasticDemodSettings::serialize() const
{
    ...
    s.writeString(33, m_meshtasticKeySpecList);
    s.writeBool(34, m_meshtasticAutoSampleRate);
    s.writeString(35, m_meshtasticRegionCode);
    s.writeString(36, m_meshtasticPresetName);
    s.writeS32(37, m_meshtasticChannelIndex);

    return s.final();
}

bool MeshtasticDemodSettings::deserialize(const QByteArray& data)
{
    ...
    if ((d.getVersion() == 1) || (d.getVersion() == 2) || (d.getVersion() == 3))
    {
        ...
        d.readString(33, &m_meshtasticKeySpecList, "");
        d.readBool(34, &m_meshtasticAutoSampleRate, true);
        d.readString(35, &m_meshtasticRegionCode, "US");
        d.readString(36, &m_meshtasticPresetName, "LONG_FAST");
        d.readS32(37, &m_meshtasticChannelIndex, 0);
...
void MeshtasticDemodSettings::applySettings(const QStringList& settingsKeys, const MeshtasticDemodSettings& settings)
{
    ...
    if (settingsKeys.contains("meshtasticKeySpecList"))
        m_meshtasticKeySpecList = settings.m_meshtasticKeySpecList;
    if (settingsKeys.contains("meshtasticAutoSampleRate"))
        m_meshtasticAutoSampleRate = settings.m_meshtasticAutoSampleRate;
...
QString MeshtasticDemodSettings::getDebugString(const QStringList& settingsKeys, bool force) const
{
    QString debug;
    ...
    if (force || settingsKeys.contains("meshtasticKeySpecList"))
        debug += QString("MeshtasticKeySpecList: %1 ").arg(m_meshtasticKeySpecList);
    if (force || settingsKeys.contains("meshtasticAutoSampleRate"))
        debug += QString("MeshtasticAutoSampleRate: %1 ").arg(m_meshtasticAutoSampleRate);
    if (force || settingsKeys.contains("meshtasticRegionCode"))
        debug += QString("MeshtasticRegionCode: %1 ").arg(m_meshtasticRegionCode);
    if (force || settingsKeys.contains("meshtasticPresetName"))
        debug += QString("MeshtasticPresetName: %1 ").arg(m_meshtasticPresetName);
    if (force || settingsKeys.contains("meshtasticChannelIndex"))
        debug += QString("MeshtasticChannelIndex: %1 ").arg(m_meshtasticChannelIndex);
...
unsigned int MeshtasticDemodSettings::getNbSFDFourths() const
{
...
bool MeshtasticDemodSettings::hasSyncWord() const
{
