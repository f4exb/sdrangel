...
void InmarsatDemodSink::sampleToScopeB(Complex rrc, Complex tedError, Complex tedErrorSum, Complex ref, Complex derot, int bit, Real mu)
{
    if (m_scopeSink)
    {
        ...
        m_sampleBuffer[10][m_sampleBufferIndexB] = Complex(bit, mu);
...
void InmarsatDemodSink::processOneSample(Complex &ci)
{
    ...
    for (int i = 0; i < n_out; i++)
    {
        ...
        sampleToScopeB(rrc, m_error, m_errorSum, m_ref, m_derot, m_bit*2-1, m_mu);
    }
}
...
void InmarsatDemodSink::getPLLStatus(bool &locked, Real &coarseFreqCurrent, Real &coarseFreqCurrentPower, Real &coarseFreq, Real &coarseFreqPower, Real &fineFreq) const
{
    locked = m_locked;
