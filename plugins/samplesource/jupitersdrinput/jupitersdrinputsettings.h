...
#ifndef _JUPITERSDR_JUPITERSDRINPUTSETTINGS_H_
#define _JUPITERSDR_JUPITERSDRINPUTSETTINGS_H_

#include <QtGlobal>
#include <QString>
#include <stdint.h>

struct JupiterSDRInputSettings {
	typedef enum {
		FC_POS_INFRA = 0,
		FC_POS_SUPRA,
		FC_POS_CENTER,
	...
	JupiterSDRInputSettings();
	void resetToDefaults();
	QByteArray serialize() const;
	bool deserialize(const QByteArray& data);
    static void translateRFPath(RFPath path, QString& s);
    static void translateGainMode(GainMode mod, QString& s);
    void applySettings(const QStringList& settingsKeys, const JupiterSDRInputSettings& settings);
    QString getDebugString(const QStringList& settingsKeys, bool force=false) const;
};

#endif /* _JUPITERSDR_JUPITERSDRINPUTSETTINGS_H_ */
