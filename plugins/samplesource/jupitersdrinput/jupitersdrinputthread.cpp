...
JupiterSDRInputThread::JupiterSDRInputThread(uint32_t blocksizeSamples, DeviceJupiterSDRBox* plutoBox, SampleSinkFifo* sampleFifo, QObject* parent) :
    QThread(parent),
    m_running(false),
    m_plutoBox(plutoBox),
    ...
    m_log2Decim(0),
    m_fcPos(JupiterSDRInputSettings::FC_POS_CENTER),
    m_phasor(0),
    m_iqOrder(true)
{
...
JupiterSDRInputThread::~JupiterSDRInputThread()
{
    stopWork();
    delete[] m_buf;
}

void JupiterSDRInputThread::startWork()
{
...
void JupiterSDRInputThread::stopWork()
{
...
void JupiterSDRInputThread::setLog2Decimation(unsigned int log2_decim)
{
    m_log2Decim = log2_decim;
}

void JupiterSDRInputThread::setFcPos(int fcPos)
{
    m_fcPos = fcPos;
}

void JupiterSDRInputThread::run()
{
    std::ptrdiff_t p_inc = m_plutoBox->rxBufferStep();

    qDebug("JupiterSDRInputThread::run: rxBufferStep: %ld bytes", p_inc);
    qDebug("JupiterSDRInputThread::run: Rx sample size is %ld bytes", m_plutoBox->getRxSampleSize());
    qDebug("JupiterSDRInputThread::run: Tx sample size is %ld bytes", m_plutoBox->getTxSampleSize());
    qDebug("JupiterSDRInputThread::run: nominal nbytes_rx is %d bytes with 1 refill", m_blockSizeSamples*4);
    ...
    while (m_running)
    {
        ...
        {
            qWarning("JupiterSDRInputThread::run: error refilling buf %d / %d",(int) nbytes_rx, (int)  m_blockSizeSamples*4);
            usleep(200000);
            continue;
        }
...
void JupiterSDRInputThread::convertIQ(const qint16* buf, qint32 len)
{
...
void JupiterSDRInputThread::convertQI(const qint16* buf, qint32 len)
{
    SampleVector::iterator it = m_convertBuffer.begin();
