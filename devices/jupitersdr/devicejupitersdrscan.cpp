...
#include "devicejupitersdrscan.h"

void DeviceJupiterSDRScan::scan()
{
    int i, num_contexts;
    ...
    if (!scan_ctx)
    {
        qCritical("JupiterSDRScan::scan: could not create scan context");
        return;
    }
    ...
    if (num_contexts < 0)
    {
        qCritical("JupiterSDRScan::scan: could not get contexts");
        return;
    }
    ...
    for (i = 0; i < num_contexts; i++)
    {
        ...
        if (!DeviceJupiterSDRBox::probeURI(std::string(uri))) { // continue if not accessible
            continue;
        }

        qDebug("JupiterSDRScan::scan: %d: %s [%s]", i, description, uri);
        char *pch = strstr(const_cast<char*>(description), "JupiterSDR");
...
const std::string* DeviceJupiterSDRScan::getURIAt(unsigned int index) const
{
...
const std::string* DeviceJupiterSDRScan::getSerialAt(unsigned int index) const
{
    if (index < m_scans.size()) {
...
const std::string* DeviceJupiterSDRScan::getURIFromSerial(
...
void DeviceJupiterSDRScan::getSerials(std::vector<std::string>& serials) const
{
...
void DeviceJupiterSDRScan::enumOriginDevices(const QString& hardwareId, PluginInterface::OriginDevices& originDevices)
{
    scan();
	...
	for (i = 0; it != serials.end(); ++it, ++i)
	{
	    ...
	    QString displayableName(QString("JupiterSDR[%1] %2").arg(i).arg(serial_str));
        ...
        qDebug("DeviceJupiterSDRScan::enumOriginDevices: enumerated JupiterSDR device #%d", i);
	}
